#!/usr/bin/env python3
"""Reusable PPTX-first engine for two-session student notebook generation."""
...
SLIDE_H = Inches(7.5)
PUBLISHER_STYLE_VERSION = "publisher-2026-04"
FONT_HEAD = "Calibri"
FONT_BODY = "Calibri"
...
NAVY = RGBColor(28, 46, 66)
TEAL = RGBColor(56, 127, 132)
GOLD = RGBColor(220, 177, 91)
CORAL = RGBColor(209, 108, 88)
SAGE = RGBColor(137, 163, 115)
INK = RGBColor(36, 42, 48)
MUTED = RGBColor(92, 101, 110)
LINE = RGBColor(214, 219, 223)
SOFT_LINE = RGBColor(196, 205, 214)
SOFT_NAVY = RGBColor(71, 93, 116)
PAPER_WARM = RGBColor(252, 250, 246)
MARGIN_ROSE = RGBColor(229, 198, 190)
PALE_BLUE = RGBColor(234, 242, 247)
PALE_GOLD = RGBColor(252, 245, 224)
PALE_CORAL = RGBColor(252, 236, 230)
PALE_SAGE = RGBColor(237, 244, 233)
PALE_NAVY = RGBColor(232, 237, 243)
...
REFERENCE_WORKBOOK_BASELINE_GUIDANCE = (
    "Start from the compact workbook baseline, then adapt it to the uploaded lesson slides instead of inventing a new notebook structure each time. "
    "The default model should be a premium six-slide notebook for each session: Objectives + Session Map, Be Curious, Vocabulary + Reference Tool, Guided Problem, Interactive Activity, and Best-Fit Interactive Review. "
    "Problem pages should show the actual lesson problem, give students visible space to solve it, preserve the source wording when available, and keep the interactive work tightly anchored to the lesson language, visuals, and setup. "
    "Each session should show a visible engagement arc, not just polished static pages: a source-faithful launch, a useful reference page, one guided problem, one lesson-type interactive task, and one discussion-ready review."
)
EXACT_ESOL_WORKBOOK_GUIDANCE = (
    "Use the compressed hybrid notebook format as the default direction: premium lesson-specific structure without filler pages. "
    ...
    "Use this exact six-slide architecture for each session: Objectives + Session Map, Be Curious / Notice-Wonder, Vocabulary + Reference Tool, Guided Problem, Interactive Activity, Best-Fit Interactive Review. "
    ...
    "Avoid worksheet crowding, decorative clutter, and flattened image-only pages."
)
PREMIUM_NOTEBOOK_GUIDANCE = (
    ...
    "Default to the locked six-slide notebook architecture for each session: Objectives + Session Map, Be Curious, Vocabulary + Reference Tool, Guided Problem, Interactive Activity, and Best-Fit Interactive Review. "
    "Adapt the notebook structure to the uploaded lesson instead of forcing one exact workbook template for every deck. "
    ...
    "Student engagement is non-negotiable: avoid decks that only ask students to read, write, and solve in static boxes. Build recurring opportunities to sort, compare, fix, teach, and revise thinking from the exact source problems. "
    ...
    "Integrate written source problems directly into the guided problem and interactive pages instead of paraphrasing them away. "
...
def build_planner_instructions(custom_guidance: str = "") -> str:
    ...
    instructions = (
        "You are designing a premium two-session student notebook from an extracted lesson slide deck. "
        "Preserve the lesson's real topic, sequence, academic language, visuals, worked examples, and practice when present. "
        "Do not replace the lesson with generic filler. Split Session 1 toward launch and concept building, and Session 2 toward review, deeper practice, application, and closure. "
        ...
        f"{REFERENCE_WORKBOOK_BASELINE_GUIDANCE} "
        "Keep the locked notebook architecture intact through the session flow: Cover, Learning Target, Notice-Wonder, Vocabulary, Guided Notes, Guided Practice, Collaborative Practice, Independent Practice, Reflection, and Exit Ticket. "
        "Aim for 13 to 15 slides per session when the source deck has enough substance, while staying within the required 10 to 16 slide range. "
        "Each session should feel like a complete premium resource with enough value to stand alone: strong cover, launch, objective check-in, vocabulary support, notes, modeled work, source-faithful practice, interactivity, and closure. "
        ...
        "Preserve the lesson's original problems, but transform only the strongest pages into premium interactive notebook experiences so the deck feels polished instead of crowded. "
        ...
        "Integrate some of the written source problems directly into worked example, practice, challenge, and exit ticket pages instead of only paraphrasing them in supporting text. "
        ...
        "It is acceptable to add extra premium notebook slides when the source deck contains additional strong written problems, visual models, or comparison opportunities worth surfacing. "
        "Each session should contain between 10 and 16 slides. "
        "Session 1 and Session 2 must each include a learning_target slide. "
        "For a learning_target slide, primary_text must be the content objective and secondary_text must be the language objective. "
        "Each learning_target slide must preserve the actual lesson objective language from the source, include 3 to 4 success criteria, and support a before-and-after self-assessment with draggable checkmarks for both objectives. "
        "For the learning_target slide, movable_pieces should include repeated checkmarks and activity_instructions should explain placing them in before/after boxes for the content and language objectives. "
        ...
        "Build in Writing Revolution-style kernel sentence frames, not just loose words, especially on guided_notes, worked_example, practice, reflection, and exit_ticket slides. "
        ...
        "Apply a premium decision layer instead of random novelty. Choose only the strongest 2 to 4 premium enhancements per session from this set when the lesson supports them: error analysis, multi-representation builder, scenario continuity, scaffold fade, strategy comparison, decision tree, Writing Revolution explanation, turn-and-teach prompt, create-your-own transfer task, and reflection/mastery tracker. "
        "Do not force every premium feature into every lesson. Prefer clarity and coherence over novelty, and keep every premium move tied to the exact lesson objective, task type, and source context. "
        ...
        "Use named activities on about 4 to 5 strong slides per session when the source lesson supports it, not on every page. "
        "Keep covers and objective pages clean and premium rather than forcing a separate activity panel onto them. "
        "Use varied interaction types from the database so the notebook feels rich and premium, not repetitive. Avoid repeating the same activity_name within a session when strong alternatives exist. "
        "Give the notebook stronger editorial design energy: bold but classroom-friendly cover pages, image-led anchors when the source deck supports them, and varied compositions across notes, worked examples, practice, and reflection pages. "
        "When source images exist, use them beyond the cover whenever they strengthen modeling, comparison, or vocabulary understanding. "
...
def build_responses_payload(
    ...
    user_content: list[dict[str, Any]] = [
        {
            "type": "input_text",
            "text": (
                "Use the extracted source deck below to plan a reusable two-session notebook.\n\n"
    ...
    return {
        ...
        "text": {
            "format": {
                "type": "json_schema",
                "name": "two_session_notebook_plan",
                "strict": True,
...
def validate_plan(plan: dict[str, Any], deck: dict[str, Any] | None = None) -> dict[str, Any]:
    for key in ("lesson_title", "subject", "grade_level", "standards", "topic_summary", "session_1", "session_2"):
        if key not in plan:
    ...
    for session_key in ("session_1", "session_2"):
        ...
        min_slides = len(EXACT_ESOL_TEMPLATE_SEQUENCE) if uses_exact_esol_template(session) else MIN_SESSION_SLIDES
        max_slides = len(EXACT_ESOL_TEMPLATE_SEQUENCE) if uses_exact_esol_template(session) else MAX_SESSION_SLIDES
        ...
        if practice_like < 2:
            if uses_exact_esol_template(session):
                raise RuntimeError(f"{session_key} must include both the interactive activity and best-fit review slides.")
            raise RuntimeError(f"{session_key} must include both collaborative and independent practice coverage.")
...
def closing_slide_allowed(plan_slide: dict[str, Any], session: dict[str, Any] | None = None) -> bool:
    if session and uses_exact_esol_template(session):
        return (
...
def select_session_premium_features(
    ...
    if len(selected) < 2:
        for fallback in ("error_analysis", "multi_representation", "turn_and_teach", "writing_revolution"):
...
def run_formal_release_plan_review(plan: dict[str, Any], *, deck: dict[str, Any] | None = None) -> None:
    ...
    for session_key in ("session_1", "session_2"):
        ...
        if uses_exact_esol_template(session):
            if slides[0].get("kind") != "learning_target":
                issues.append(f"{session_key}: first slide must be learning_target")
        elif slides[0].get("kind") != "cover":
            issues.append(f"{session_key}: first slide must be cover")
        if not closing_slide_allowed(slides[-1], session):
            issues.append(f"{session_key}: last slide does not match the locked closing requirement")
...
def build_exact_esol_workbook_session(
    ...
    data_analysis_focus = has_data_analysis_context(combined_source_text(problem_records))
    ...
    *,
    ...
    plan = {
        ...
        "session_1": {
            ...
            "session_subtitle": "Launch, build understanding, and capture the first round of notes.",
            "slides": session1_slides,
        },
        "session_2": {
            "session_title": "Session 2 Student Notebook",
            "session_subtitle": "Review, apply, extend, and close the lesson.",
...
def add_card(
    ...
    *,
    ...
    band_height = Inches(0.08) if compact else Inches(0.10)
    band = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.RECTANGLE, x, y, w, band_height)
    band.fill.solid()
    ...
    add_text(
        slide,
        x + Inches(0.18),
        y + (Inches(0.14) if compact else Inches(0.20)),
        w - Inches(0.36),
        Inches(0.18) if compact else Inches(0.34),
        title,
        size=min(title_size, 11.8) if compact else title_size,
        min_size=10.8 if compact else 13.6,
        color=accent,
        bold=True,
        font=FONT_HEAD,
        margin=0.02,
    ...
    if body:
        body_top = y + (Inches(0.34) if compact else Inches(0.60))
        body_height = max(int(h - (Inches(0.40) if compact else Inches(0.78))), 1)
        add_text(
            slide,
            x + Inches(0.18),
            body_top,
...
def add_tag(slide: Any, x: int, y: int, text: str, fill_color: RGBColor) -> None:
    width = max(float(Inches(1.55)), min(float(Inches(2.75)), float(Inches(0.92 + max(len(normalize_whitespace(text)), 8) * 0.065))))
...
def add_footer_bar(slide: Any, footer_text: str) -> None:
    if not normalize_whitespace(footer_text):
        return
    footer = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.RECTANGLE, Inches(0.34), Inches(7.03), Inches(12.65), Inches(0.05))
    footer.fill.solid()
...
def add_header(
    ...
    *,
    ...
    band = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.RECTANGLE, 0, 0, SLIDE_W, Inches(0.42))
    band.fill.solid()
    band.fill.fore_color.rgb = NAVY
    band.line.fill.background()
    accent_rule = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.RECTANGLE, 0, Inches(0.42), SLIDE_W, Inches(0.04))
    accent_rule.fill.solid()
    accent_rule.fill.fore_color.rgb = accent
    accent_rule.line.fill.background()
    add_tag(slide, Inches(0.48), Inches(0.66), truncate_text(section, 28), accent)
    add_text(
        slide,
        Inches(0.48),
        Inches(0.98),
...
def add_lined_area(
    ...
    *,
    ...
    frame = add_rect(slide, x, y, w, h, fill, line_color=SOFT_LINE)
...
def render_exact_cover_slide(
    ...
    *,
    ...
    set_background(slide)
    top_band = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.RECTANGLE, 0, 0, SLIDE_W, Inches(0.58))
    top_band.fill.solid()
    ...
    add_card(
        ...
        Inches(1.08),
        "Notebook Promise",
        truncate_text(plan_slide.get("primary_text", "") or "Use the source lesson and solve in the notebook.", 108),
        fill=PALE_BLUE,
        accent=TEAL,
        title_size=13.4,
        body_size=12.0,
    ...
    add_rect(slide, Inches(6.10), Inches(4.00), Inches(6.63), Inches(2.32), PAPER, line_color=TEAL)
    add_text(
        slide,
        Inches(6.28),
        Inches(4.14),
    ...
    toc_items = plan_slide.get("toc_items", [])
    for index, item in enumerate(toc_items[:12]):
        col = 0 if index < 6 else 1
        row = index if index < 6 else index - 6
        add_text(
            slide,
            Inches(6.28) + col * Inches(3.06),
            Inches(4.46) + row * Inches(0.28),
            Inches(2.78),
            Inches(0.18),
            truncate_text(item, 34),
            size=10.8,
            color=INK,
            font=FONT_BODY,
            margin=0.01,
...
def render_cover_slide(
    ...
    *,
    ...
    set_background(slide)
    add_rect(slide, Inches(6.96), Inches(0.72), Inches(5.9), Inches(5.05), PALE_GOLD, line_color=PALE_GOLD)
    hero = add_rect(slide, Inches(0.45), Inches(0.62), Inches(6.35), Inches(5.78), PAPER)
    hero.adjustments[0] = 0.18
    ...
    ribbon.fill.solid()
    ribbon.fill.fore_color.rgb = CORAL
    ...
    title_rule = slide.shapes.add_shape(
        MSO_AUTO_SHAPE_TYPE.RECTANGLE,
        Inches(0.82),
        Inches(3.00),
        Inches(1.92),
    ...
    title_rule.fill.fore_color.rgb = CORAL
    ...
    add_text(
        ...
        Inches(5.2),
        Inches(0.34),
        truncate_text(session_label, 60),
        size=12.6,
        color=CORAL,
        bold=True,
        font=FONT_HEAD,
...
def render_html_notebook(plan: dict[str, Any], deck: dict[str, Any], output_dir: Path) -> Path:
    ...
    session_html = (
        render_html_session(plan["session_1"], session_label="Session 1", image_lookup=image_lookup)
        + render_html_session(plan["session_2"], session_label="Session 2", image_lookup=image_lookup)
    )
    html = f"""<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    :root {{
      ...
      --line:rgba(31,39,49,.10);
      --shadow:0 24px 70px rgba(54,46,31,.12);
      --radius-xl:30px;
      --radius-lg:22px;
      --radius-md:16px;
    ...
    * {{ box-sizing:border-box; }}
    ...
    body {{
      ...
      color:var(--text);
      background:
        radial-gradient(circle at top left, rgba(198,91,61,.10), transparent 26%),
        radial-gradient(circle at 85% 8%, rgba(56,127,132,.11), transparent 22%),
        linear-gradient(180deg, #fffaf3 0%, var(--bg) 42%, var(--bg-2) 100%);
    }}
    a, button {{
...
def build_rendered_quality_report(plan: dict[str, Any], outputs: dict[str, Path]) -> dict[str, Any]:
    ...
    session_specs = (
        ("session_1", "Session 1", "session1"),
        ("session_2", "Session 2", "session2"),
    )
    for session_key, label, output_key in session_specs:
        session_plan = plan.get(session_key, {})
        slide_texts = extract_pptx_slide_texts(outputs[output_key])
...
def render_plan(
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    session1_path = output_dir / "Session 1 - Student Notebook.pptx"
    session2_path = output_dir / "Session 2 - Student Notebook.pptx"
    render_session_notebook(
        plan["session_1"],
        session_label="Session 1",
        lesson_title=plan["lesson_title"],
        deck=deck,
        output_path=session1_path,
    )
    render_session_notebook(
        plan["session_2"],
        ...
        lesson_title=plan["lesson_title"],
        deck=deck,
        output_path=session2_path,
    )
    return {"session1": session1_path, "session2": session2_path}
...
def cmd_extract(args: argparse.Namespace) -> int:
    source_pptx = Path(args.source).expanduser().resolve()
    output_dir = resolve_output_dir(source_pptx, Path(args.output_dir).expanduser() if args.output_dir else None)
...
def cmd_render(args: argparse.Namespace) -> int:
    ...
    output_dir = plan_path.parent if not args.output_dir else Path(args.output_dir).expanduser().resolve()
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate two-session student notebooks from a source PPTX deck."
    )
    ...
    plan_parser = subparsers.add_parser("plan", help="Create a two-session notebook plan JSON.")
